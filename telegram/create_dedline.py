...
async def load_time(message: types.Message, state: FSMContext):  # Получение времени
    from datetime import datetime
    ...
    try:  # Проверка на доступность даты
        ...
        try:
            ...
            await FSMCreate.next()
            await message.reply(
                'Теперь введите как часто повторять данный дедлайн (одноразовый - введите "один", каждую ниделю - введите "часто")', reply_markup=kb_reg)
...
async def load_regularity(message: types.Message, state: FSMContext):  # Получение регулярности дедлайнов и завершение
    ...
    if message.text.lower() == 'часто' or message.text.lower() == 'один':
        async with state.proxy() as data:
            data['ded_regularity'] = message.text.lower()
        await FSMCreate.next()
        await message.reply(
            """Введите дату, когда вас предупредить о дедлайне формата ЧЧ.ММ.ГГГГ. В настройках вы сможете добавить ещё одну точку""", reply_markup=kb_client)
    else:
        await message.reply('Вы ввели неправильный формат даты или эта дата недоступна!')
